...
double pos_origin[3] = {-2.9, 0, 0};
double pos_est[6] ; //0-1 is pos by gps, 2-3 is by acc, 4-5 is by encoder
double err_loc[3] = {0,0,0}; //metrics for gps, acc and encoder
double avg_err_loc[3] = {0,0,0}; //average localization error
double pos_true_prev[2] = {-2.9, 0}; //previous position of rob (the estimate we receive is one step later)
static FILE *fp;
...
int main(int argc, char *args[]) {
  ...
  fprintf(fp, "time; err_gps; err_acc; err_enc; err_gps_avg; err_acc_avg; err_enc_avg\n");
    ...
    if(time_now_s>0){
      ...
      if( fp != NULL){
        fprintf(fp, "%g; %g; %g; %g; %g; %g; %g\n",
            time_now_s, err_loc[0],err_loc[1],err_loc[2],avg_err_loc[0],avg_err_loc[1],avg_err_loc[2]);
      }
    }

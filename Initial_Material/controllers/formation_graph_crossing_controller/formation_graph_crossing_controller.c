...
- make it working with crossing
- pay attention to init theta for different groups
/*****************************************************************************/
...
#define FLOCK_SIZE	  10 // Size of formation
#define GROUP_SIZE 5 // Size of formation group
...
int group_id; // 0-4: group0, 5-9: group1

/**************************************************/
/* ROBOT VARIABLES INITIALIZATION */
/**************************************************/
int robot_id_u, robot_id;	// Unique and normalized (between 0 and FLOCK_SIZE-1) robot ID
float relative_pos[GROUP_SIZE][3];	// relative X, Z, Theta of all robots
float prev_relative_pos[GROUP_SIZE][3];	// Previous relative  X, Z, Theta values
float my_position[3];     		// X, Z, Theta of the current robot
float prev_my_position[3];  		// X, Z, Theta of the current robot in the previous time step
float true_position[GROUP_SIZE][3]; // true position for algorithm test
float global_position[GROUP_SIZE][3]; // true position for algorithm test
float speed[GROUP_SIZE][2];		// Speeds calculated with graph-based formation
float final_speed[GROUP_SIZE][2];
float goal_speed[2];
...
void init_device(int time_step){
	...
	robot_id = robot_id_u;	  // normalize between 0 and FLOCK_SIZE-1
                 // init group_id
                 if (robot_id < 5){
                     group_id = 0;
                 } else if (robot_id >=5) {
...
void process_received_ping_messages(void) {
	...
	while (wb_receiver_get_queue_length(receiver) > 0) {
		...
		if (inbuffer[0] != 'e'){
			...
			sscanf(inbuffer,"%1d#%f#%f#%f",
			&i,&true_position[i%GROUP_SIZE][0],&true_position[i%GROUP_SIZE][1],&true_position[i%GROUP_SIZE][2]);
			wb_receiver_next_packet(receiver);
			true_position[i/GROUP_SIZE][2] += M_PI/2;
			limit_rad(true_position[i/GROUP_SIZE][2]);
		...
		if (other_robot_id/GROUP_SIZE != group_id){
		    continue;
		}

		int other_id = other_robot_id%GROUP_SIZE;

		// Get position update
		prev_relative_pos[other_id][0] = relative_pos[other_id][0];
		prev_relative_pos[other_id][1] = relative_pos[other_id][1];

		relative_pos[other_id][1] = -range*sinf(theta);
		relative_pos[other_id][0] = range*cosf(theta);  // relative x pos

		relative_pos[other_id][0] *= -1.0;
		relative_pos[other_id][1] *= -1.0;

		printf("RobotTheta %f\t", theta);
		global_position[other_id][0] = my_position[0] + relative_pos[other_id][0];
		global_position[other_id][1] = my_position[1] + relative_pos[other_id][1];
		// if (VERBOSE){
...
void update_self_motion(int msl, int msr) {
    ...
    int id = robot_id%GROUP_SIZE;

	limit_rad(_odo_enc.heading);
	// use true position
	if (USE_TRUE){
	    my_position[0] = true_position[id][0];
	    my_position[1] = true_position[id][1];
	    my_position[2] = true_position[id][2] + M_PI/2.0;
	}
	else {
	    // Use KF results
	    if (ODO_TYPE == 1){
	...
	printf("(Global) Current robot %d: (%f, %f)\n", robot_id, global_position[id][0], global_position[id][1]);
...
void compute_wheel_speeds(int *msl, int *msr, gsl_matrix * laplacian){
	...
	int id = robot_id%GROUP_SIZE;
	speed[id][0] = 0; speed[id][1] = 0;

	// robot_id: 10-robot crossing
	for(int j = 0; j < GROUP_SIZE; j++){
		// only need to show available connection
		if (j == id) {
			// printf("true_position: (%f, %f)\n", true_position[id][0], true_position[id][1]);
			// true position
			if (USE_TRUE){
			    speed[id][0] -= gsl_matrix_get (laplacian, id, j) * (true_position[j][0] - bias_x[j]);
			    speed[id][1] -= gsl_matrix_get (laplacian, id, j) * (true_position[j][1] - bias_y[j]);
                                                   }
                                                   else {
                                                       // global position
                                                       speed[id][0] -= gsl_matrix_get (laplacian, id, j) * (global_position[j][0] - bias_x[j]);
			    speed[id][1] -= gsl_matrix_get (laplacian, id, j) * (global_position[j][1] - bias_y[j]);
			}
		}
		else {
			if (gsl_matrix_get (laplacian, id, j) == 0){
				continue;
			}
	...
	if (VERBOSE_SPEED){
  	    printf ("X-lap speed vs goal speed: (%f, %f) \n", speed[id][0], goal_speed[0]);
  	    printf ("Y-lap speed vs goal speed: (%f, %f) \n", speed[id][1], goal_speed[1]);
	}
        	if (LEADER_MODE == true){
				if (robot_id % 5 == 0){
				final_speed[id][0] = goal_speed[0];
				final_speed[id][1] = - goal_speed[1];
				}
				else{
					final_speed[id][0] = speed[id][0];
					final_speed[id][1] = - speed[id][1];
				}
        	}
        	else {
				final_speed[id][0] = speed[id][0] + goal_speed[0];
				final_speed[id][1] = -speed[id][1] - goal_speed[1];
			}
	...
	if (USE_TRUE){
		...
		x = final_speed[id][0]*cosf(true_position[id][2] - 1.57) + final_speed[id][1]*sinf(true_position[id][2] - 1.57); // x in robot coordinates
		y = -final_speed[id][0]*sinf(true_position[id][2] - 1.57) + final_speed[id][1]*cosf(true_position[id][2] - 1.57); // y in robot coordinates
	} else {
		printf("robot %d true heading %f\n", robot_id, true_position[id][2]);
		...
		float tmp_x = final_speed[id][0];
		float tmp_y = final_speed[id][1];
		final_speed[id][0] = -tmp_y;
		final_speed[id][1] = tmp_x;
		x = final_speed[id][0]*cosf(global_position[id][2])  + final_speed[id][1]*sinf(global_position[id][2]); // x in robot coordinates
		y = -final_speed[id][0]*sinf(global_position[id][2] ) + final_speed[id][1]*cosf(global_position[id][2]); // y in robot coordinates
...
int main(){
	...
	gsl_matrix * laplacian  = gsl_matrix_alloc (GROUP_SIZE, GROUP_SIZE);

...
// Flocking parameters
#define VERBOSE_flocking true       // Compute flocking metrics
#define FLOCKING_DIST     0.14   // Flcoking pairwise robot distance

// Formation parameter
// Following value should be in Webots coordination!!!
#define VERBOSE_formation false       // Compute formation metrics
...
// The parameter "1.22" is obtained by making the robot move forwards with fastes speed
float max_dis = DELTA_T*(MAX_SPEED_WEB*MAX_SPEED*WHEEL_RADIUS/1000)*1.22; //maximal distance possible per timestep
...
/*
 * Send true position of robots (For controller debug only)
 */
void send_true(){
...
void compute_metric_flocking(){
  ...
  if (VERBOSE_flocking_metric){
    printf("orientation metric of group %d is %g      ",super_id,orientation);
    ...
    printf("distance metric of group %d is %g     ",super_id,distance);
    printf("velocity metric of group %d is %g \n",super_id,velocity);
    printf("overall metric of group %d is %g \n",super_id,metric);
  }
  if( fp_flocking != NULL){
...
void compute_metric_formation(){
  ...
  if (VERBOSE_formation_metric){
    printf("distance metric of group %d is %g \n",super_id,distance);
    printf("velocity metric of group %d is %g \n",super_id,velocity);
    printf("overall metric of group %d is %g \n",super_id,metric);
  }
...
int main(int argc, char *args[]) {
  ...
  if (VERBOSE_flocking){
    sprintf(file_name1,"flocking_metrics%d.csv",super_id);
    fp_flocking = fopen(file_name1,"w");
    fprintf(fp_flocking, "time; orientation; distance; velocity towards goal direction; overall\n");
  }
  
  if (VERBOSE_formation){
    sprintf(file_name2,"formation_metrics%d.csv",super_id);
    fp_formation = fopen(file_name2,"w");

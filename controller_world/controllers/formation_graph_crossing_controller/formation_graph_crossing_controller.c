...
#define GROUP_SIZE  5 // Size of formation
#define EDGE_SIZE	  4 // Size of edges
#define AXLE_LENGTH          0.052     // Distance between wheels of robot
#define SPEED_UNIT_RADS      0.00628    // Conversion factor between speed unit to radian per second
#define TS			0.064			// Timestep (seconds)
 ...
 * 1: ACC-based KF
 * 2: ENC-based KF
 * 3: ACC-based position+ENC-based heading
 * 4: naive odom
*/
#define ODO_TYPE 2
#define USE_LOC_TRUE false // true false
#define USE_REL_TRUE false // true false
...
// first five robots
float bias_x[10] = {0,      0,     0,     0,     0,     2.8,    2.8,    2.8,    2.8,    2.8};
float bias_y[10] = {0.0,  -0.1,   0.1, -0.2,  0.2,   0.0,   0.1, -0.1, 0.2, -0.2};
...
// kalman filter matrices for covariances and state (for accelerometer and encoder based)
...
void limit_speed_f(float *number, float limit) {
	if (*number > limit)
		*number = limit;
	if (*number < -limit)
		*number = -limit;
...
void init_position_kf(int time_step, double x_init, double z_init, double h_init)
{
	...
	_odo_enc.y= _pose.y;
	if (robot_id >= 5 ){
	    _pose_origin.heading= _pose.heading+M_PI;
	    _odo_acc.heading= _pose.heading+M_PI;
	    _odo_enc.heading= _pose.heading+M_PI;
	} else{
...
void initial_pos(){
	while (initialized[robot_id] == 0) {
		...
		printf("robot %d, current (%f, %f), goal (%f, %f)\n",
		robot_id, my_position[0], my_position[1], goal_pos[0], goal_pos[1]);
...
void process_received_ping_messages(void) {
	...
	int other_robot_id, current_group, other_group;
	while (wb_receiver_get_queue_length(receiver) > 0) {
		...
		other_group = other_robot_id/5;
		current_group = robot_id/5;
		if (other_group == current_group){
		   int id = other_robot_id%5;
		...
		relative_pos[id][1] = -range*sinf(theta);
		relative_pos[id][0] = range*cosf(theta);  // relative x pos

		relative_pos[id][0] *= -1.0;
		relative_pos[id][1] *= -1.0;

		global_position[id][0] = my_position[0] + relative_pos[id][0];
		global_position[id][1] = my_position[1] + relative_pos[id][1];
		}
		wb_receiver_next_packet(receiver);
	}
}
...
void update_self_motion(int msl, int msr) {
	...
	if (USE_LOC_TRUE){
		...
		my_position[2] = limit_rad(_odo_acc.heading);
			...
			float dr = (float)msr * SPEED_UNIT_RADS * WHEEL_RADIUS * TS;
			float dl = (float)msl * SPEED_UNIT_RADS * WHEEL_RADIUS * TS;
			float du = (dr + dl)/2.0;
	...
	int id = robot_id%5;
	global_position[id][0] = my_position[0];
	global_position[id][1] = my_position[1];
	// new: update errors
	prev_err[0] = err[0]; prev_err[1] = err[1];
	err[0] = goal_pos[0] - my_position[0];
	err[1] = goal_pos[1] - my_position[1];
...
void compute_wheel_speeds(int *msl, int *msr, gsl_matrix * laplacian){
	...
	for(int j = 0; j < GROUP_SIZE; j++){
		...
		if (j == id) {
			/* true position */
			if (USE_LOC_TRUE){
			    speed[id][0] -= gsl_matrix_get (laplacian, id, j) * (true_position[j+group_id*5][0] - bias_x[j]);
			    speed[id][1] -= gsl_matrix_get (laplacian, id, j) * (true_position[j+group_id*5][1] - bias_y[j]);
                                                   }
                                                   else {
                                                       /* global position */
  	    ...
  	    printf ("Y-lap speed vs goal speed: (%f, %f) \n", speed[id][1], goal_speed[1]);
        	...
        	if (LEADER_MODE == true){
				...
				final_speed[id][1] = -speed[id][1] - goal_speed[1];
                 ...
                 float x, y;
	if (USE_REL_TRUE){
		x = final_speed[id][0]*cosf(true_position[robot_id][2] - 1.57) + final_speed[id][1]*sinf(true_position[robot_id][2] - 1.57); // x in robot coordinates
		y = -final_speed[id][0]*sinf(true_position[robot_id][2] - 1.57) + final_speed[id][1]*cosf(true_position[robot_id][2] - 1.57); // y in robot coordinates
	} else {
...
int main(){
	...
	while (true) {
		...
		speed[robot_id][0] = (1/TS)*(my_position[0]-prev_my_position[0]);
		speed[robot_id][1] = (1/TS)*(my_position[1]-prev_my_position[1]);
		...
		limit_speed_f(&msl_w, MAX_SPEED_WEB);
		limit_speed_f(&msr_w, MAX_SPEED_WEB);
		if (VERBOSE_SPEED){
